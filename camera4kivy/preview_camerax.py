...
from kivy.graphics import Fbo, Callback, Rectangle, Rotate, Scale, Translate, Color
...
class PreviewCameraX(PreviewCommon, CommonGestures):
    ...
    def connect_camera(
        ...
        **kwargs,
        ...
        print(f"%%%% self.canvas_callback {self.canvas_callback} %%%%")
        ...
        self.cb_wrapper = CallbackWrapper(
            self._filename_callback, self._analyze_image_proxy, self._configure_pipeline
        )
    ...
    def _configure_camera(self, start):
        ...
        if self._camera:
            ...
            with self.canvas:
                ...
                print(f"view size? {self.view_size} ; pos? {self.view_pos}")
    ...
    def capture_photo(self, location="", subdir="", name=""):
        if self._camera:
            ...
            app.image_path = os.path.join(subdir, name)
            if self.file_storage:
                self._name_pipe.append(join(subdir, name))
            self._camera.capture_photo(subdir, name, self.file_storage)

    # def capture_photo(self, location="", subdir="", name=""):
    #     print("CAPTURING PHOTO ON PREVIEW_CAMERAX.PY")
    #     if self._camera:
    #         self._set_location("private")
    #         subdir = self._default_subdir_android(subdir)
    #         name = self._default_file_name(name, ".jpg")
    ...
    #         if self.file_storage:
    #             self._name_pipe.append(join(subdir, name))
    #         print("CAPTURING PHOTO ON PREVIEW_CAMERAX.PY 2")
    #         self._camera.capture_photo(subdir, name, False)
    #         print("CAPTURING PHOTO ON PREVIEW_CAMERAX.PY 3")

    #         from kivy.app import App

    #         app = App.get_running_app()
    #         gallery_screen = app.screen_manager.get_screen("Gallery Screen")
    #         import os
    ...
    #         # gallery_screen.encrypt_image(full_name)
    ...
    #         # print("self._camera.texture: ", self._camera.texture)
    #         # print(
    #         #     "self._camera.texture.get_region: ",
    #         #     self._camera.texture.get_region(*self.tex_crop),
    #         # )
    ...
    def _create_fbo(self, texture_size, rotation):
        ...
        with self._fbo:
            self._camera_texture_cb = Callback(lambda instr: self._camera_texture.bind)
    ...
    def _create_pipeline(self, texture_size, rotation):
        ...
        if self._camera:
            print("check no setTexture aplicado")
    ...
    def _pipeline_thread(self):
        fps = 30
        self._update_ev = Clock.schedule_interval(self._update_pipeline, 1 / fps)
    ...
    def _analyze_texture(self):
        if not self.enable_data and self._analyze_callback:
            ...
            self._analyze_callback(
                tex, self.view_pos, self.tscale, self.facing == "front"
            )
