...
class Preview(AnchorLayout):
    ...
    def __init__(self, **kwargs):
        print("esse é o preview modificado")
        ...
        for key in ["letterbox_color", "aspect_ratio", "orientation"]:
            if key in kwargs:
    ...
    def connect_camera(
        ...
        **kwargs
        ...
        self.preview.connect_camera(
            analyze_callback=self.analyze_image_callback_schedule,
            analyze_proxy_callback=self.analyze_imageproxy_callback,
            canvas_callback=self.canvas_instructions_callback,
            **kwargs
        )
    ...
    def analyze_image_callback_schedule(self, texture, tpos, tscale, mirror):
        ...
        if not self._busy:
            ...
            if self.analyzer_resolution:
                ...
                fbo_scale = max(
                    texture.width / fbo_size[0], texture.height / fbo_size[1]
                )
                fbo_pos = (
                    (texture.width - fbo_size[0] * fbo_scale) / 2,
                ...
                fbo_size = (
                    round(texture.size[0] / fbo_scale),
    ...
    def image_scheduler(self):
        while True:
            ...
            # Thread
            self.analyze_pixels_callback(
                self.pixels, self.im_size, self.tpos, self.scale, self.mirror
            )
            self._busy = False
    ...
    def analyze_pixels_callback(
        self, pixels, image_size, image_pos, image_scale, mirror
    ):
        pass
    ...
    def analyze_imageproxy_callback(
        self, image_proxy, image_pos, image_scale, mirror, degrees
    ):
        pass
