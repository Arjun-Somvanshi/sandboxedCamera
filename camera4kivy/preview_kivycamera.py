...
class PreviewKivyCamera(PreviewCommon):
    ...
    def capture_photo(self, location=".", subdir="", name=""):
        print("capture_photo on preview_kivycamera.py", flush=True)
        from icecream import ic

        if self._camera and self._camera.texture:
            path = self.capture_path(location, subdir, name)
            tex = self._camera.texture.get_region(*self.tex_crop)
            print("tex_crop", self.tex_crop, flush=True)
            print("tex_size", tex.size, flush=True)
            print("tex", tex, flush=True)
    ...
    def camera_error_message(self):
        self.error_message = (
            "WARNING: Unable to connect to camera_id '"
            + str(self.index)
            + "'.\n"
    ...
    def on_tex(self, camera):
        if self._camera:
            ...
            if self.data_callback:
                self.data_callback(tex, self.view_pos, self.tscale, self.mirror)
            if self.mirror:
    ...
    def canvas_text(self, text):
        ...
        if label.texture:
            pos = [
                self.view_pos[0] + (self.view_size[0] - label.texture.size[0]) / 2,
